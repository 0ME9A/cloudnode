...
export function Cobe({
  ...
  useEffect(() => {
    ...
    const globe = createGlobe(canvasRef.current, {
      ...
      markers: (() => {
        // If the caller passed locations with coordinates, always use those
        const customWithCoords = locations.filter(
          (l) => l.lat !== undefined && l.long !== undefined,
        );
        if (customWithCoords.length > 0) {
          return customWithCoords.map((l) => ({
            location: [l.lat!, l.long!] as [number, number],
            size: markerSize,
          }));
        }

        // rotate-to-location uses geocoded locations (set via state)
        ...
        // Default fallback — hardcoded demo world cities
        if (
          variant === "default" ||
          variant === "draggable" ||
